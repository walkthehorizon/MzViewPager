...
public class LoopViewPager extends ViewPager {
    ...
    public void setAdapter(PagerAdapter adapter) {
        ...
        setCurrentItem(getItemFirstAppearPosition(0), false);
    }
    ...
    public int getCurrentItem() {
        return mAdapter != null ? mAdapter.toInnerPosition(super.getCurrentItem()) : 0;
    }
...
//        int realItem = getItemFirstAppearPosition(item);
     ...
     * 通过该方法寻找初始位置及重置位置
     * */
    public int getItemFirstAppearPosition(int realPosition) {
        if (!mBoundaryLooping) {
            return realPosition;
        }
        for (int i = mAdapter.getCount() / 2; i < mAdapter.getCount(); i++) {
            if (mAdapter.toInnerPosition(i) == realPosition) {
                return i;
            }
        }
    ...
    private OnPageChangeListener onPageChangeListener = new OnPageChangeListener() {
        ...
        public void onPageSelected(final int position) {
            ...
            int realPosition = mAdapter.toInnerPosition(position);
            if (position == 0 || position == mAdapter.getCount() - 1) {
                setCurrentItem(getItemFirstAppearPosition(realPosition),false);
            }
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            ...
            if (mAdapter != null) {
                realPosition = mAdapter.toInnerPosition(position);
...
//                    setCurrentItem(realPosition, false);
            ...
            if (mOnPageChangeListeners != null) {
                for (int i = 0; i < mOnPageChangeListeners.size(); i++) {
                    OnPageChangeListener listener = mOnPageChangeListeners.get(i);
                    if (listener != null) {
                        if (realPosition != mAdapter.getInnerCount() - 1) {
        ...
        public void onPageScrollStateChanged(int state) {
            if (mAdapter != null) {
                ...
                if (state == ViewPager.SCROLL_STATE_IDLE && (position == 1
                    ...
                    setCurrentItem(getItemFirstAppearPosition(mAdapter.toInnerPosition(position)), false);
                }
            }
